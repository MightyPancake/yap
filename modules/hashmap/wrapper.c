...
static void hashmap_set_allocator(void *(*malloc)(size_t), void (*free)(void*)) {
    __malloc = malloc;
    __free = free;
}
...
static void hashmap_set_grow_by_power(struct hashmap *map, size_t power) {
...
static void hashmap_set_load_factor(struct hashmap *map, double factor) {
...
static struct hashmap *hashmap_new_with_allocator(void *(*_malloc)(size_t), 
...
static struct hashmap *hashmap_new(size_t elsize, size_t cap, uint64_t seed0, 
    uint64_t seed1,
    uint64_t (*hash)(const void *item, uint64_t seed0, uint64_t seed1),
    int (*compare)(const void *a, const void *b, void *udata),
    void (*elfree)(void *item),
...
// will not be called
static const void *hashmap_set_with_hash(struct hashmap *map, const void *item,
    uint64_t hash)
{
    hash = clip_hash(hash);
    map->oom = false;
...
static const void *hashmap_set(struct hashmap *map, const void *item) {
...
// own hash. The 'hash' callback provided to the hashmap_new function
// will not be called
static const void *hashmap_get_with_hash(struct hashmap *map, const void *key, 
    uint64_t hash)
{
    hash = clip_hash(hash);
...
static const void *hashmap_get(struct hashmap *map, const void *key) {
...
static const void *hashmap_probe(struct hashmap *map, uint64_t position) {
...
// own hash. The 'hash' callback provided to the hashmap_new function
// will not be called
static const void *hashmap_delete_with_hash(struct hashmap *map, const void *key,
    uint64_t hash)
{
    hash = clip_hash(hash);
    map->oom = false;
...
static const void *hashmap_delete(struct hashmap *map, const void *key) {
...
static bool hashmap_iter(struct hashmap *map, size_t *i, void **item) {
...
static uint64_t hashmap_sip(const void *data, size_t len, uint64_t seed0,
    uint64_t seed1)
{
...
static uint64_t hashmap_murmur(const void *data, size_t len, uint64_t seed0,
    uint64_t seed1)
{
    (void)seed1;
...
static uint64_t hashmap_xxhash3(const void *data, size_t len, uint64_t seed0,
    uint64_t seed1)
{
    (void)seed1;
...
static uint64_t hm_hash_8(const void *item, uint64_t seed0, uint64_t seed1) {
    return hashmap_xxhash3(item, 8, seed0, seed1);
}
static int hm_compare_4(const void *a, const void *b, void *udata) {
    (void)udata;
    return memcmp(a, b, 4);
}
static int hm_compare_8(const void *a, const void *b, void *udata) {
    (void)udata;
    return memcmp(a, b, 8);
}
...
static int hm_compare_cstr(const void *a, const void *b, void *udata) {
    (void)udata;
...
struct hashmap *yhm_make(uint64_t elsize, int32_t hash_id, int32_t cmp_id) {
    uint64_t (*hash_fn)(const void *, uint64_t, uint64_t) =
        hash_id == 0 ? hm_hash_4 : hash_id == 1 ? hm_hash_8 : hm_hash_cstr;
    int (*cmp_fn)(const void *, const void *, void *) =
        cmp_id == 0 ? hm_compare_4 : cmp_id == 1 ? hm_compare_8 : hm_compare_cstr;
...
void yhm_free(struct hashmap *map) {
    hashmap_free(map);
}

void yhm_clear(struct hashmap *map, bool update_cap) {
    hashmap_clear(map, update_cap);
}

uint64_t yhm_count(struct hashmap *map) {
    return (uint64_t)hashmap_count(map);
}

bool yhm_oom(struct hashmap *map) {
    return hashmap_oom(map);
}

const void *yhm_get(struct hashmap *map, const void *item) {
    return hashmap_get(map, item);
}

const void *yhm_set(struct hashmap *map, const void *item) {
    return hashmap_set(map, item);
}

const void *yhm_delete(struct hashmap *map, const void *item) {
    return hashmap_delete(map, item);
}

bool yhm_iter(struct hashmap *map, size_t *i, void **item) {
